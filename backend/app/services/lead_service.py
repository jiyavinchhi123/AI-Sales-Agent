...
class LeadService:
    def ensure_demo_lead(self, db: Session, user_id: str):
        ...
        if is_fashion:
            ...
            matched_offering = seller_products if seller_products else "Bandhani Kurtis, Modal Silk Sarees & Festive Lehengas"
            prod_lead = seller_products_list[0] if seller_products_list else "Handcrafted Bandhani Kurtis & Sarees"
            req_title = f"Bulk seasonal procurement for {prod_lead}"
            req_desc = f"High-end ethnic fashion retail chain & boutique distribution network seeking direct manufacturer supply of authentic {matched_offering} with verified bulk delivery capacity."
